...
def initialize_log_file():
    if not os.path.exists(LOG_FILE):
        with open(LOG_FILE, 'w', newline='') as file:
            ...
            writer.writerow(['Timestamp', 'Date', 'Person Name'])

def log_recognition(person_name):
    ...
    with open(LOG_FILE, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([timestamp, date, person_name])
        ...
        if any(matches):
            ...
            if matches[best_match_index] and face_distances[best_match_index] < RECOGNITION_TOLERANCE:
                ...
                if (last_recognition_info is None or
                    ...
                    log_recognition(name) # This function handles the actual writing to CSV
